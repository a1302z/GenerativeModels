...
class Decoder(nn.Module):    
    def __init__(self, final_size=(28,28), in_channels=16, encode_factor=2, channel_increase_factor=2, conv_blocks_per_decrease=1, RGB=False, initial_upsample_size=3):
        super(Decoder, self).__init__()
        decode_modules = [nn.Upsample(scale_factor=initial_upsample_size)]
...
class Autoencoder(nn.Module):
    def __init__(self, variational=False, final_size=(28,28), base_channels=16, encode_factor=2, channel_increase_factor=2, conv_blocks_per_decrease=1, encoding_dimension=128, RGB=False, initial_upsample_size=3):
        ...
        self.decoder = Decoder(final_size=final_size, in_channels=self.final_channels, encode_factor=encode_factor, channel_increase_factor=channel_increase_factor, conv_blocks_per_decrease=conv_blocks_per_decrease, RGB=RGB, initial_upsample_size=initial_upsample_size)
...
if __name__ == '__main__':
    ...
    if args.model == 'test':
        ...
        model = Autoencoder(variational=args.model=='VAE', final_size=(28,28), base_channels=base_channels, encode_factor=encode_factor, channel_increase_factor=channel_increase_factor, conv_blocks_per_decrease=conv_blocks_per_decrease, encoding_dimension=encoding_dimension, RGB=False, initial_upsample_size=initial_upsample_size)
        
    model.to(device)
    summary(model, (1, 28, 28), batch_size=args.batch_size)
